...
## 20. CUDA Graph 与 `torch.compile`：执行重放与计算优化

### 两者分别优化什么？

| 机制 | 主要作用 | 直观理解 |
|---|---|---|
| `torch.compile` | 编译优化函数中的计算，可能融合算子、减少中间数据读写与 kernel 启动次数 | 优化“怎么算” |
| CUDA Graph | 捕获 GPU 运算及依赖，后续整体重放，减少 CPU 逐次提交运算的开销 | 优化“怎么启动这些计算” |

这里的“图”不是图片，而是运算及其依赖关系。`torch.compile` 不局限于优化某个节点，也可以优化整段计算图、改变节点划分；CUDA Graph 主要记录和重放执行流程，并不意味着它会自动融合图中的计算。

### CUDA Graph 的“捕获与重放”是什么意思？

普通执行时，CPU 为模型的一系列 GPU 运算逐次发出指令。即使提交是异步的，不需要每次等 GPU 算完，发指令本身仍有开销。

```text
普通执行：每轮由 CPU 逐次提交 A、B、C、D 等运算

图捕获：记录 GPU 运算、依赖关系以及使用的内存地址
后续重放：更新输入缓冲区 → 启动整张图 → GPU 按依赖执行运算
```

重放复用的是执行安排，不是上一次的输出。上一轮输入 token A，本轮换成 token B，GPU 仍然重新计算 B 的结果。当前实现将新输入和上下文信息复制到图使用的固定缓冲区，再调用 `graph.replay()`；它还为多个 batch 大小保存图，运行时选择能容纳当前 batch 的图。

Decode 每轮每条序列只处理一个新 token，提交开销可能占比较高，因此减少这部分开销有助于加速。实际收益取决于工作负载，不保证每种情况下都更快。

### `@torch.compile` 在采样器中的作用
...
它让 PyTorch 对采样函数尝试编译优化。这里的计算包括温度缩放、softmax、随机数生成、除法和 argmax；哪些操作能融合、能减少多少开销，由编译器与运行条件决定，不能假定全部融合为一个 kernel。

- 首次调用通常需要追踪和编译，开销较大；后续可复用编译结果。
- 输入形状等条件变化时，可能重新编译，并非每次形状变化都必然重编译。
- 编译不会把第一次的随机数固定下来，随机采样仍会生成新的随机数。

### 它们是“底座与上层”，或“图与单个节点”吗？

这两种比喻都不够准确。两者可以独立使用，也可以配合，没有固定的上下层依赖关系。例如，一种配合方式是：

```text
原始计算：          A → B → C → D
compile 优化后：    [ABC 融合 kernel] → D
CUDA Graph：       捕获优化后的执行流程，后续整体启动
```

这只是解释两种机制如何配合的示意，不代表当前项目已将采样器和模型前向捕获成同一张图。当前项目显式捕获的是模型前向，语言头计算和采样调用在该图之外；采样器单独使用 `@torch.compile`。

### `enforce_eager` 控制哪一部分？

```python
self.enforce_eager = config.get('enforce_eager', False)
```

`eager` 表示即时执行：运行到运算时直接发起执行；它不表示每一步都同步等待 GPU 完成。

| 配置 | 模型初始化 | Prefill 前向 | Decode 前向 |
|---|---|---|---|
| `True` | 跳过显式 CUDA Graph 捕获 | 普通执行 | 普通执行 |
| `False`（默认） | 捕获 CUDA Graph | 普通执行 | 图重放 |

默认路径通常付出更多初始化时间及图相关内存开销，换取后续 decode 的提交开销降低。持续运行时更容易摊薄初始化成本；很短的任务总耗时未必更低。

**本项目的 `enforce_eager` 只控制模型显式 CUDA Graph 路径，不会自动关闭采样器上的 `@torch.compile`。** 因此设置为 True 不能理解为整个程序完全没有编译优化。

### 什么时候使用这些优化？

选择依据是实际瓶颈和重复执行次数：时间主要花在计算与中间数据读写，还是 CPU 发起 GPU 运算的开销上？后续调用能否摊薄编译或捕获成本？

| 场景 | 优先考虑 |
|---|---|
| 反复执行的一段张量计算包含许多零散算子，有融合和减少中间读写的空间 | `torch.compile` |
| GPU 运算流程重复，形状与内存地址可安排为稳定缓冲区，CPU 提交开销明显 | CUDA Graph |
| 两方面开销都有，且执行路径兼容 | 两者配合，再通过测量确认收益 |
| 只执行少量次数，或正在频繁修改、调试代码 | 先用普通执行，避免优化准备成本干扰判断 |

在当前项目中：

- **采样器使用 `torch.compile`**：每轮都要执行温度缩放、softmax、随机数生成、除法和 argmax，可尝试编译优化，重复调用摊薄编译成本。
- **Decode 使用 CUDA Graph**：模型层和执行流程反复出现，每条序列每轮只输入一个 token，适合考察提交开销是否显著。代码为多个 batch 大小捕获图，以适应请求数量变化。
- **Prefill 使用普通执行**：输入长度变化较大，计算量通常也更大。当前项目没有为它显式捕获图，这是实现选择，不表示 prefill 原理上不能使用编译或图重放优化。

实践顺序：

1. 先用普通执行确认结果正确，理解并调试完整流程。
2. 测量实际耗时，找到反复执行的热点，再决定采用哪种优化。
3. 对比稳定运行耗时、首次编译或捕获成本、显存占用；结合实际运行次数判断总收益。

若要建立完全不使用这两种机制的普通执行基线，除了设置 `enforce_eager=True`，还需单独处理采样器的 `@torch.compile`。不能只改该配置就将其称为完全未编译的基线。
